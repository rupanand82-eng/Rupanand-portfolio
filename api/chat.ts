...
export default async function handler(req: any, res: any) {
  // CORS configuration to allow Vercel previews and cross-origin interactions safely
  ...
  try {
    ...
    try {
      ...
      return res.status(500).json({
        error: "Gemini client credentials missing on server. Ensure GEMINI_API_KEY is defined in Vercel Environment Variables.",
        details: envErr.message
      });
    }

    // Map history to Gemini chat shape
    ...
    return res.status(200).json({ reply: geminiResponse.text });
  } catch (error: any) {
